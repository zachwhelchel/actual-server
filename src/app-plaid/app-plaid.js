...
const configuration = new Configuration({
  basePath:
    REACT_APP_PLAID_ENVIRONMENT === 'production'
      ...
      : PlaidEnvironments.sandbox, // or development/production
  baseOptions: {
    headers: {
...
app.post('/api/create_link_token', async function (request, response) {
  ...
  const itemIdExists =
    request.body.item_id !== undefined &&
  ...
  if (itemIdExists) {
    ...
    let REACT_APP_AIRTABLE_ACCOUNT_FIELD =
      process.env.REACT_APP_AIRTABLE_ACCOUNT_FIELD;
    ...
    const plaidItemsRecords = await base('PlaidItems')
      .select({
        filterByFormula: `AND(
  ...
  try {
    ...
    for (const item of plaidItemsRecords) {
      ...
      try {
        ...
        console.error(
          `Error fetching institutions for item ${item.fields.item_id}:`,
          error,
        );
...
app.post('/institution/remove', async function (request, response) {
  try {
    ...
    return response
      .status(500)
      .json({ error: 'Error removing Plaid connection' });
  }
});
  ...
  try {
    ...
    for (const item of plaidItemsRecords) {
      ...
      try {
        ...
        const formattedAccounts = accountsResponse.data.accounts.map(
          (account) => ({
        ...
        console.error(
          `Error fetching accounts for item ${item.fields.item_id}:`,
          error,
        );
        // Continue to next item even if this one fails
      }
...
app.post(
  ...
  handleError(async (req, res) => {
    ...
    try {
      ...
      const accountRecords = await base(process.env.REACT_APP_AIRTABLE_TABLE)
        .select({
      ...
      if (accountRecords.length === 0) {
        return response.status(404).json({
          error: `${process.env.REACT_APP_AIRTABLE_TABLE} not found for current user`,
        });
      }
      ...
      const matchingRecords = plaidItemsRecords.filter((record) => {
        try {
          ...
          return (
            idsArray.includes(targetId) ||
            record.get('user_id')?.[0] === targetId
          );
        } catch (error) {
...
async function getTransactions(
  ...
  try {
    ...
    console.log(
      `Fetching transactions from ${startDateString} to ${endDateString}`,
    );
      ...
      while (hasMore) {
        ...
        const flippedTransactions = transactions.map((transaction) => {
          ...
          if (transaction.amount !== undefined && transaction.amount !== null) {
            ...
            console.log(
              'Transaction missing amount property:',
...
function getAccountResponse(results, accountId, startDate) {
  ...
  for (const trans of results.transactions.filter(
...
async function savePlaidItemToAirtable(itemId, accessToken, userId) {
  try {
    ...
    if (!itemId || !accessToken || !userId) {
      throw new Error(
        'Missing required parameters: itemId, accessToken, and userId are required',
      );
    }

    // 1. Find the StageAccount record for the current user
    const accountRecords = await base(REACT_APP_AIRTABLE_TABLE)
      .select({
        filterByFormula: `{user_id} = '${userId}'`,
        maxRecords: 1,
      })
    ...
    const existingRecords = await base('PlaidItems')
      .select({
